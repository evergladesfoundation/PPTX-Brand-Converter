...
"$PY" "$ROOT/rebrand/inspect_template.py" "$TEMPLATE" --out-dir "$OUT" ${BRAND_MD:+--brand-md "$BRAND_MD"}
"$PY" "$ROOT/rebrand/inspect_source.py" "$SOURCE" --out-dir "$OUT"
"$PY" "$ROOT/rebrand/plan.py" --out-dir "$OUT"
"$PY" "$ROOT/rebrand/build.py" --out-dir "$OUT" --source "$SOURCE" --template "$TEMPLATE"
"$PY" "$ROOT/rebrand/qa.py" --out-dir "$OUT" --source "$SOURCE"
